...
Description: Centralized logging configuration for SentinelAI framework
"""
...
def _iter_managed_loggers() -> Iterable[logging.Logger]:
    seen = set()
    for logger_name in ("sentinelai., *logging.Logger.manager.loggerDict.keys()):
        if logger_name in seen:
            continue
        seen.add(logger_name)
        if logger_name == "sentinelai. or logger_name.startswith(_MANAGED_LOGGER_PREFIXES):
...
def _install_buffer_handler_if_enabled(logger: logging.Logger) -> None:
    ...
    try:
        from sentinelai.utils.log_buffer import install_buffer_handler
        install_buffer_handler(logger=logger)
    except Exception:
        logger.debug("Failed to install SentinelAI log buffer handler", exc_info=True)


def get_logger(name: str = "sentinelai.) -> logging.Logger:
    """
    Get a configured logger instance for SentinelAI framework.

    Args:
        name: Logger name (defaults to 'sentinelai.)
...
def set_log_level(level: str) -> None:
    """
    Set the log level for all SentinelAI loggers.
...
def set_log_format(format_name: str) -> None:
    """
    Set the output format for all managed SentinelAI-related loggers.
