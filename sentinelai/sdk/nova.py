"""
SentinelAI SDK Main Class

High-level SDK for SentinelAI prompt pattern matching with policy-based actions.
"""
...
from sentinelai.evaluators.llm import get_validated_evaluator, LLMEvaluator
from sentinelai.evaluators.condition import can_llm_change_outcome
from sentinelai.utils.helpers import normalize_unicode

from .policy import SentinelPolicy, Action
from .result import ScanResult, RuleMatch
from .redaction import Redactor
from .exceptions import SentinelBlockedError, SentinelConfigError


class Sentinel:
    """
    High-level SDK for SentinelAI prompt pattern matching.
    ...
    Example:
        ...
        result = sentinelai.scan(user_prompt)
        if result.blocked:
    ...
    def __init__(
        ...
        rules: Optional[List[SentinelRule]] = None,
        policy: Optional[Union[Dict, SentinelPolicy]] = None,
        ...
        """
        Initialize SentinelAI SDK.

        Args:
            rules_path: Path(s) to .nov rule files or directories
            rules: Pre-loaded SentinelRule objects
            policy: Policy configuration (dict or SentinelPolicy)
            ...
            ignore_invalid_rules: Skip invalid rule files instead of raising SentinelConfigError
        ...
        self._debug = debug
        self._matchers: Dict[str, SentinelMatcher] = {}
        ...
        if isinstance(policy, SentinelPolicy):
            ...
            self._policy = SentinelPolicy(policy, default_action=default_action)
        else:
            self._policy = SentinelPolicy(default_action=default_action)
        ...
        for path in paths:
            ...
            if path.is_file():
                ...
                if not rule_files and not self._ignore_invalid_rules:
                    raise SentinelConfigError(f"No .nov rule files found under rules_path: {path}")
                for rule_file in rule_files:
                    self._load_rule_file(rule_file, parser)
            elif not self._ignore_invalid_rules:
                raise SentinelConfigError(f"Rules path does not exist or is not readable: {path}")

    def _load_rule_file(self, path: Path, parser: SentinelParser) -> None:
        ...
        if len(self._rules) == loaded_before and not self._ignore_invalid_rules:
            raise SentinelConfigError(f"No valid rules loaded from {path}")
    ...
    def _handle_rule_load_error(self, path: Path, message: str, error: Exception) -> None:
        ...
        if self._ignore_invalid_rules:
            return
        raise SentinelConfigError(f"Failed to load rule file {path}: {message}") from error

    def _append_rule(self, rule: SentinelRule, source: Optional[Path] = None) -> None:
        """Append a rule while preserving unique rule names."""
        if any(existing.name == rule.name for existing in self._rules):
            raise SentinelConfigError(f"Duplicate rule name loaded: {rule.name}")
    ...
    def _initialize_evaluators(self) -> None:
        ...
        if needs_semantic:
            try:
                from sentinelai.evaluators.semantics import DefaultSemanticEvaluator, _EMBEDDING_CACHE
    ...
    def _create_configured_llm_evaluator(self) -> Optional[LLMEvaluator]:
        ...
        try:
            ...
            if self._llm_provider:
                raise SentinelConfigError(f"Failed to initialize LLM evaluator: {e}") from e
            # Preserve historical behavior for default OpenAI: no key means LLM
            # rules fail closed instead of blocking keyword-only use.
            return None

    def _rule_needs_llm(self, rule: SentinelRule) -> bool:
    ...
    def _initialize_matchers(self) -> None:
        ...
        for rule in self._rules:
            self._matchers[rule.name] = SentinelMatcher(
                rule=rule,
    ...
    def protect(
        ...
        """
        Decorator to protect a function with SentinelAI scanning.
        ...
        Args:
            ...
            raise_on_block: Whether to raise SentinelBlockedError
        ...
        Example:
            ...
            @sentinelai.protect(action="block", skip_llm=True)
    ...
    @property
    def rules(self) -> List[SentinelRule]:
