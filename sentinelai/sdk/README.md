...
from sentinelai.sdk import SentinelAI, SentinelAIBlockedError
...
except SentinelAIBlockedError as e:
...
```
User Input → SentinelAI Scan → Policy Check → Action
                              ↓
                    block / flag / redact / allow
```

1. **User sends a prompt**
2. **SentinelAI scans** against loaded rules (jailbreak, injection, etc.)
...
|--------|-------------|--------|
| `"block"` | Stop the request | `result.blocked = True`, raises `SentinelAIBlockedError` with decorator |
...
When SentinelAI finds a match (e.g., rule named `PromptInjectionJailbreak`):
...
result = sentinelai.scan("user input here")
...
sentinelai = SentinelAI(rules_path="sentinelai-rules/", policy={...})
...
```python
# Async scanning
result = await sentinelai.scan_async("user input")
...
```python
result = sentinelai.scan("some text")
...
sentinelai = SentinelAI(
    # Rules
    rules_path="sentinelai-rules/",           # Path to .nov files or directory
...
When `llm_model` is omitted, SentinelAI checks provider-specific model environment variables such as `OPENROUTER_LLM_MODEL` and `OPENROUTER_MODEL`, then falls back to `SentinelAI_LLM_MODEL` and the provider default.
...
@sentinelai.protect(
    ...
    raise_on_block=True,      # Raise SentinelAIBlockedError if blocked
...
# Run specific test
pytest tests/test_sdk.py::TestSentinelAIPolicy -v
```
