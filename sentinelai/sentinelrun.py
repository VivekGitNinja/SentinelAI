...
Description: Command-line tool for running SentinelAI rules against prompts
"""
...
try:
    ...
    from sentinelai.utils.config import get_config
    from sentinelai.evaluators.llm import get_validated_evaluator
except ImportError:
    print("Error: SentinelAI package not found in PYTHONPATH.")
    print("Make sure SentinelAI is installed or set your PYTHONPATH correctly.")
    sys.exit(1)
...
def load_rule_file(file_path: str) -> str:
    """
    Load a SentinelAI rule from a file.
...
def extract_rules(content: str) -> List[str]:
    ...
    """
    return SentinelRuleFileParser()._extract_rule_blocks_optimized(content)
...
def check_if_rule_needs_llm(rule) -> bool:
    ...
    Args:
        rule: The parsed SentinelAI rule
...
def check_if_rules_need_llm(rules) -> bool:
    ...
    Args:
        rules: List of parsed SentinelAI rules
...
def process_prompt(rule_text: str, prompt: str, verbose: bool = False, 
    ...
    Args:
        rule_text: SentinelAI rule definition
...
def process_rule(rule, prompt: str, verbose: bool = False,
    ...
    matcher = SentinelMatcher(rule, llm_evaluator=llm_evaluator)
...
def print_llm_reasons(result: Dict[str, Any]) -> None:
    """Print LLM reason fields captured by SentinelMatcher."""
...
def print_evaluation_warnings(result: Dict[str, Any]) -> None:
    """Print fail-closed or degraded-evaluation warnings from SentinelMatcher."""
...
def main():
    """Main entry point for the sentinelai.un tool."""
    parser = argparse.ArgumentParser(
        description="SentinelAI Rule Runner - Check prompts against SentinelAI rules"
    )
    
    parser.add_argument('-r', '--rule', required=True, help='Path to the SentinelAI rule file')
    ...
    parser.add_argument('-c', '--config', help='Path to SentinelAI configuration file')
    ...
    if not args.single and len(rule_blocks) > 1:
        ...
        try:
            parsed_rules = SentinelRuleFileParser().parse_content(file_content, args.rule)
        except Exception as e:
