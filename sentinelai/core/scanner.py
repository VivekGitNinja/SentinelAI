...
Description: Scanner for checking prompts against multiple SentinelAI rules
"""
...
from sentinelai.evaluators.llm import OpenAIEvaluator, LLMEvaluator, get_validated_evaluator
from sentinelai.utils.logger import get_logger
from sentinelai.utils.helpers import normalize_unicode

# Get logger for this module
logger = get_logger("sentinelai.scanner")

class SentinelScanner:
    """
    Scanner that checks prompts against multiple SentinelAI rules.
    """
    
    def __init__(
        self,
        rules: List[SentinelRule] = None,
        ...
        Args:
            rules: List of SentinelRule objects to check against (optional)
            llm_type: Optional LLM provider for rules that require LLM evaluation.
                      Supported values match sentinelai.un: openai, anthropic, azure,
    ...
    def _validate_unique_rule_names(self, rules: List[SentinelRule]) -> None:
    ...
    def _create_matcher(self, rule: SentinelRule) -> SentinelMatcher:
    ...
    def add_rule(self, rule: SentinelRule) -> None:
        ...
        Args:
            rule: SentinelRule object to add
    ...
    def add_rules(self, rules: List[SentinelRule]) -> None:
        """
        Add multiple rules to the scanner.
        
        Args:
            rules: List of SentinelRule objects to add
