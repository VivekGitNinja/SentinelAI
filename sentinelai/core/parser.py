...
Description: SentinelAI rule parser with grammar validation
"""

import re
from typing import Dict, List
from sentinelai.core.rules import SentinelRule, KeywordPattern, SemanticPattern, LLMPattern
from sentinelai.utils.logger import get_logger

# Get logger for this module
logger = get_logger("sentinelai.parser")
...
class SentinelParserError(Exception):
    """Exception raised for SentinelAI rule syntax errors."""
    pass


class SentinelParser:
    """Parser for SentinelAI rule files with grammar validation."""
    ...
    def parse(self, content: str) -> SentinelRule:
        """
        Parse a SentinelAI rule definition string into a SentinelRule object.
        
        Args:
            content: The rule definition string
            
        Returns:
            A SentinelRule object representing the parsed rule
            
        Raises:
            SentinelParserError: If the rule definition doesn't follow the grammar
        """
        ...
        self.rule = SentinelRule(name=rule_name)
    ...
    def _validate_variable_name(self, key: str, section: str, line_num: int) -> None:
        """Validate a SentinelAI variable name."""
    ...
    def _parse_condition_section(self, content: List[str]) -> str:
        ...
        Raises:
            SentinelParserError: If the condition syntax is invalid
        """
        ...
        if not raw_condition:
            raise SentinelParserError("Condition section cannot be empty")
    ...
    def _validate_rule_structure(self):
        ...
        Raises:
            SentinelParserError: If the rule structure is invalid
        """
        try:
            ...
            try:
                ...
                if not error_message.startswith(f"Rule '{self.rule.name}'"):
                    raise SentinelParserError(f"Rule '{self.rule.name}': {error_message}")
                raise
            ...
            try:
                ...
                if re.search(r'\bof\s+\(\s*(keywords|semantics|llm)\.\$[^)]*\)', cleaned_condition):
                    raise SentinelParserError(f"Rule '{self.rule.name}': Invalid syntax: 'of (keywords.$var*)'. Use section prefix in the variable reference instead (e.g., 'of keywords.$var*')")
                ...
                if re.search(r'\(\s*(keywords|semantics|llm)\.\*\s*\)', cleaned_condition):
                    raise SentinelParserError(f"Rule '{self.rule.name}': Invalid wildcard syntax: section wildcards should not be enclosed in parentheses. Use 'keywords.*' instead of '(keywords.*)'")
                ...
                # Skip the compile check if the condition has SentinelAI-specific syntax
                has_sentinelai.specific_syntax = False
                if SECTION_QUANTIFIER_PATTERN.search(cleaned_condition) or re.search(r'\b(any|all|\d+)\s+of\s+', cleaned_condition) or '.*' in cleaned_condition:
                    has_sentinelai.specific_syntax = True
                
                # Only do the compile check if there's no SentinelAI-specific syntax
                if not has_sentinelai.specific_syntax:
                    try:
                        ...
                        raise SentinelParserError(error_message)
                        
            except SentinelParserError:
                # Re-raise SentinelParserError without modification
                raise
            ...
            # Re-raise SentinelParserError without modification (we've already added rule name)
            raise
        except Exception as e:
            # For any other unexpected errors, add the rule name
            raise SentinelParserError(f"Rule '{self.rule.name}': Unexpected error: {str(e)}")
    ...
    def _validate_condition_variables(self):
        ...
        Raises:
            SentinelParserError: If the condition references undefined variables
        """
        condition = self.rule.condition
...
# Backward-compatible re-export: SentinelRuleFileParser historically lived in this
# module and is still imported from sentinelai.core.parser by downstream users.
from sentinelai.core.rule_file import SentinelRuleFileParser as SentinelRuleFileParser  # noqa: E402
