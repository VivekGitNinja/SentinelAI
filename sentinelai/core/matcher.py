...
Description: Core matcher implementation for SentinelAI rules
"""
...
from sentinelai.evaluators.keywords import DefaultKeywordEvaluator
...
logger = get_logger("sentinelai.matcher")
...
def _get_llm_evaluator_classes():
    ...
    if not _imports_attempted['llm']:
        ...
        try:
            from sentinelai.evaluators.llm import OpenAIEvaluator, LLMEvaluator
...
class SentinelMatcher:
    ...
    def __init__(self, 
        ...
        Args:
            rule: The SentinelRule to match against
        ...
        if needs_semantic:
            if semantic_evaluator:
                ...
                if SemanticEvalClass is not None:
                    ...
                    logger.warning("Rule requires semantic evaluation but sentence-transformers not available. Install with: pip install sentinelai.hunting")
        else:
            self.semantic_evaluator = None
        ...
        if llm_evaluator:
            ...
            if OpenAIEval is not None:
                ...
                logger.warning("Rule requires LLM evaluation but LLM dependencies not available. Install with: pip install sentinelai.hunting")
        else:
