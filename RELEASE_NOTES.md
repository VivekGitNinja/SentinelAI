# SentinelAI Hunting v0.3.0 Release Notes

## Highlights
- Promoted SentinelAI from beta to production/stable status, reflecting the hardened fail-closed engine, full CI/security gates, and stability guarantees established over the 0.2.x series.
...
- Restructured the LLM evaluator module into a per-provider package (`sentinelai/evaluators/llm/`) and moved the multi-rule file parser into `sentinelai/core/rule_file.py`. All public import paths, including `sentinelai.evaluators.llm` and `sentinelai.core.parser`, are unchanged.
...
- The public package version is `0.3.0`; `sentinelai.__version__` and package metadata share a single source of truth in `sentinelai/_version.py`.
- Supported Python versions are 3.10 and newer.
- Contributors: the local gates now lint `sentinelai tests scripts` (the root `test.py` script no longer exists).
